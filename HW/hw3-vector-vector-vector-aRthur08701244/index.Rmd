...
下方為 2020 總統大選某投開票所開出的前 100 張選票 (儲存於 `votes`)，請將 `votes` 裡面的每個元素的第一個字母改成大寫 (e.g. `tsai` --> `Tsai`)。(hint: 你可以使用 `unique()` 找出 `votes` 裡面有哪些候選人)
    ...
    votes[votes=="gou"]<-("Gou")
    votes[votes=="ko"]<-("Ko")
    votes[votes=="wang"]<-("Wang")
    votes[votes=="tsai"]<-("Tsai")
    votes[votes=="han"]<-("Han")
...
#x<-rep(c(LETTERS), times = c(3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3))
#y<-rep(c(1:3),times=26)
#paste0(x,"_",y)
# write your code here
AAAtoZZZ <- rep(LETTERS, 
                times = rep(3, times = length(LETTERS)))
paste0(AAAtoZZZ, '_', 1:3)
# should print out:
...
# 請在下方寫下你的程式碼
dice <- 1:6
sampling_exp <- replicate(1000, 
                          sum(sample(dice, size = 2, replace = TRUE))  # `replace = TRUE` is needed
                          )
sampling_exp
