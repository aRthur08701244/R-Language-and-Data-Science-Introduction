...
info2phrases <- function(someone, last_phrase) {
  last_phrase<-c("is quite young.","should be wise.")
 for (name in names(member)) {
    someone <- member[[name]]
  if(someone$age>35)
    last_phrase<-last_phrase[2]
  else
    last_phrase<-last_phrase[1]
...
請從 `df` 中 (1) 篩選出 `Species` 變項為 `versicolor` 的所有觀察值，並 (2) 只保留 `Sepal.Length`, `Sepal.Width`, `Petal.Length`, `Petal.Width` 這 4 個變項：
    ```{r}
    # Write your code here
    #for(i in 1:150){if(df[i,5] == "versicolor")print(df[i,1:4])}
    df[df$Species == "versicolor", -5]
...
請從 `df` 中篩選出 (1) `Species` 變項為 `versicolor` 且 (2) `Sepal.Length` 大於 5.8 的所有觀察值：
    ```{r}
    # Write your code here
    #for(i in 1:150){if(df[i,5] == "versicolor")if(df[i,1] > 5.8)print(df[i,1:4])}
    df[(df$Species == "versicolor") & (df$Sepal.Length > 5.8), ]
...
請從 `df` 中篩選出 `Sepal.Length` 與 `Sepal.Width` 之和大於 8.5 的所有觀察值。
    ```{r}
    # Write your code here
    #for(i in 1:150){if(df[i,1]+df[i,2]>8.5)print(df[i,])}
      df[(df$Sepal.Length + df$Sepal.Width > 8.5), ]
...
```{r}
# Write your code here
GOT_cast_birthday<-GOT_cast_birthday
append("Arthur",GOT_cast_birthday[36,1])
append("Arthur",GOT_cast_birthday[36,2])
append("2001/6/28",GOT_cast_birthday[36,3])
